...
(function () {
  ...
  const DEFAULT_API_URL =
    "https://9rpbu6svz7.execute-api.us-east-1.amazonaws.com";
  ...
  const API = {
    baseUrl() {
      ...
      const url = (fromLocal || fromConfig || DEFAULT_API_URL).trim();

      return url.replace(/\/$/, "").replace(/\/records$/, "");
    },
  ...
  function computeBMI() {
    ...
    if (!PatientName) {
      throw new Error("Enter patient name.");
    }
    ...
    if (heightM <= 0 || weightKg <= 0) {
      throw new Error("Height/weight must be positive.");
    }
    ...
    return {
      ...
      inputs:
        units === "metric"
          ...
          : {
              heightFt: Math.floor((heightM * 100 / 2.54) / 12),
              heightIn:
                Math.round(((heightM * 100 / 2.54) % 12) * 10) / 10,
              weightLb: Math.round((weightKg / 0.45359237) * 10) / 10,
            },
  ...
  function renderResult(result) {
    ...
    const msg =
      {
        Underweight:
          "BMI is below the healthy range. Consider nutrition review and clinical context.",
        Normal:
          "BMI is within the healthy range. Maintain lifestyle and keep tracking trends.",
        Overweight:
          "BMI is above the healthy range. Consider activity + nutrition adjustments.",
        Obesity:
          "BMI is high. Consider structured plan + clinical consultation if needed.",
      }[cat.label] || "";

    els.summary.innerHTML = `<b>${result.PatientName}</b> • ${
      result.units === "metric" ? "Metric" : "Imperial"
    } • <b>${cat.label}</b><br>${msg}`;
  ...
  async function saveToCloud() {
    ...
    if (!last) {
      throw new Error("Compute BMI first.");
    }

    const payload = JSON.parse(last);

    const body = {
      PatientName: payload.PatientName || payload.patientName,
      patientname: payload.PatientName || payload.patientName,
      units: payload.units,
    ...
    const patient = payload.PatientName || payload.patientName;
    const cached = loadCachedRecords(patient);

    const merged = [data.record, ...cached].filter(
      (v, i, arr) => i === arr.findIndex((x) => x.recordTs === v.recordTs)
    );

    saveCachedRecords(patient, merged);
  ...
  function renderRecords(records) {
    ...
    els.recordsBody.innerHTML =
      rows || `<tr><td colspan="5" class="muted">No records found.</td></tr>`;

    els.btnExport.disabled = !(records && records.length);

    els.recordsMeta.textContent =
      records && records.length ? `Loaded ${records.length} record(s).` : "";
  ...
  function drawTrend(records) {
    ...
    if (!records || records.length === 0) {
      ...
      ctx.fillText(
        "No trend to display. Search a patient to load records.",
        pad,
        pad + 20 * dpr
      );
      return;
    ...
    const pts = records.slice(0, 30).slice().reverse();

    const xFor = (i) =>
      pad + chartW * (i / Math.max(1, pts.length - 1));
  ...
  async function searchRecords() {
    ...
    if (!name) {
      throw new Error("Enter patient name to search.");
    }

    const data = await API.request(
      `/records?PatientName=${encodeURIComponent(name)}`,
      {
        method: "GET",
      }
    );
    ...
    saveCachedRecords(name, records);
  ...
  function resetSettings() {
    ...
    els.apiBaseUrl.value = DEFAULT_API_URL;
  ...
  (function init() {
    ...
    if (lastPatient) {
      els.patientName.value = lastPatient;
      els.searchName.value = lastPatient;
    }
